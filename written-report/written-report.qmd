...
glance(choco_fit_full) %>%
  select(adj.r.squared, AIC, BIC)
```
As both models have similar adjusted r squared values, and AIC and BIC values, due to the goals of parsimony and model conciseness, we choose to proceed with the first model.
